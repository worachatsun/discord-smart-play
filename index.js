...
let playlist = [ 
  ...
  {title:"Husbands - 3am", type:"local", path:"D:/Music/Jack Stauber - Coffee (extended Fan Edit) [mp3clan].mp3" , requested_by:"yacth_Mon"}
];
...
client.login('NjQ1Njg1ODIxMjQ4MTEwNjM1.XdGL7Q.0LS9YhCjUVZF0Rjo_0jppMo1JHY');
...
client.on('message', message => {
  ...
  if (this.connection !== undefined){
    if(message.content.startsWith("#play ")){
      ...
      if(query[1]){
        if( query[1] == "local"){
          let regrexArg = message.content.replace(/[\\]+/g,"/").match(/"(.*?)"/g);
          addLocal(regrexArg[0].replace(/['"]+/g, ''), regrexArg[1].replace(/['"]+/g, ''), message.author);
        } else {
          addYouTube(query[1], message.author).then(({title, requested_by, queue_number})=>{
            this.channel.send(`> Added \`${title}\` to queue \`[${queue_number}]\`, requested by \`${requested_by}\``)
          })
        }
      }
...
let play = ()=>{
  ...
  if(playlist.length >0){
    let showingType = "";
    console.log(`Playing ${playlist[0].type} mode : ${playlist[0].title}, requested by : ${playlist[0].requested_by}`);
    switch(playlist[0].type){
      case "local":
        ...
        break;
      case "youtube":
        let stream = ytdl(playlist[0].path, { filter : 'audioonly' , highWaterMark: 1<<25 });
        this.dispatcher = this.connection.playStream(stream, this.streamOptions);
        showingType = ":youtube: \`[YouTube]\`";
        break;
      default: 
        this.channel.send(`__**Something went wrong ‼**__ for this queue :: \`${playlist[0].title}\`, Type ${playlist[0].type}`);
    ...
    this.channel.send(`▶ __**Now Playing**__ :: \`${playlist[0].title}\`, Type ${showingType}`);
  ...
  return new Promise((resolve,reject)=>{
    ytdl.getInfo(url, (err,info)=>{
      ...
      if(info.title){
        ...
        return resolve({title:info.title , requested_by:requested_by.username, queue_number: playlist.length});
      }
    })
  })
}
...
let addLocal = (path, title, requested_by)=>{
  return new Promise((resolve,reject)=>{
    ...
    // console.log(requested_by);
    playlist.length == 1? play():'';
    return resolve({title , requested_by:requested_by.username, queue_number: playlist.length});
  })
}
...
let showingPlaylist = ()=>{
  if(playlist.length >0 ){
    ...
    for(let i =0,l=playlist.length;i<l;i++){
      listing += `[No.${i+1}] \`${playlist[i].title}\` on \`${playlist[i].type}\`, requested by : \`${playlist[i].requested_by}\` ${(this.playing&&i==0)? "▶ \`[Playing]\`":""}\n`;
    }
